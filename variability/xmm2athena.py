...
def _add_epoch_col(gaia_epoch=2016.0):
   ...
   edr3suss = fits.open( config['suss_summary'] )
   ...
   for k in np.arange(len(g)):
      ...
      ra_ep,dec_ep = apply_proper_motion(gk['RA_ICRS'],gk['DE_ICRS'],gaia_epoch,gk['pmRA'],
           gk['pmDE'], obs_ep)
...
def gaia_cat(root='.',o1=0,o2=10):
    ...
    (1) use obsid to find pointing RA,DEC, obs_epoch
        restrict processing epoch using [o1:o2] range
    ...
    with open("peak_in_obsid_list.txt","w") as bo:
        for res in results:
            ...
            if q.sum() > 0:
                ...
                if q.sum() == 1:
                    bo.write(f"{obses[q][0]} {mag[q][0]} {filen}\n")
                    obspeak.append(f"{obses[q][0]}")
                else:    
                   for o,mgg in zip(obses[q],mag[q]):
                      bo.write(f"{o} {mgg} {filen}\n")
                      obspeak.append(o)
...
# - - - - - - - - - 
    ...
    if len(badobs) > 0:
        with open("remove_jumpy.txt","w") as jump:
            for k in badobs:
                jump.write(f"{k}\n") 
        with open("obspeakstat.txt","w") as jump:
            for k in obspeakstat: 
                jump.write(f"{k}\n")             
            ...
            for obs,nnn in zip(obsid1,n_src1):
                 if o == obs: # match outer loop
                     ...
                     if (nnn > n_src_limit) & (ngaia1/nnn < n_var_ratio_limit):  # bad selection
                         bad_obsid.append(o)
                         bad_obsid2.append([o,nnn,ngaia1,ngaia1/nnn])
    uniq_bad_obsids=np.unique(bad_obsid)
    uniq_bad_obsid2=np.unique(bad_obsid2)
...
# - - - -    plot(n_all,n_gaia/n_all,'+g',)
    ...
    with open(indir+"bad_obsids.txt","w") as gf:
        for obs in uniq_bad_obsids:
           gf.write(f"{obs}\n")
    with open(indir+"bad_obsid_full.txt","w") as gf:
        gf.write(f"obsid  n_src  ngaia  ngaia/n_src\n")
        for obs in uniq_bad_obsid2:
           gf.write(f"{obs}\n")
...
################# end : the following were developed in searching for a fix to the WG4 astrometry problem
