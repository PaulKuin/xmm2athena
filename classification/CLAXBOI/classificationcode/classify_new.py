...
# probabilistic classification, the predicted class is the one giving the maximum 
# product likelihood*prior
# the likelihood of a class C is the probability that the source belongs to C given its 
# properties, using the distributions of each property from the objects identified 
# as C in the reference sample
...
print (f"Using {os.getcwd()} {configfile}  ")
...
# ['b', XRB] is set to 0.5*trainingsample+0.5*uniform, to work around the spikes at the 
#   galactic latitude of some nearby galaxies, which are most studied in the literature and hence more present in the reference sample
...
# adapt the reference sample to the prior proportions? Useful to get proper false positive 
#   rates. [filename]_ifnullpba.csv must exist beforehand (always run once with equipart = 0)
...
if os.path.isfile(inputfilename):
    ...
    #   to_use: whether or not the algorithm should use them in the classification 
    #   (e.g. use spectral information, do not use the source name)
    # weight: the weight of the property, representing its quality. 
    #   It will be used as an exponent to the likelihood P(value given class). 
    #   "auto" means 1/e_property will be used if e_property is a column of the catalog, 1 otherwise.
    # category: the category of the property, e.g. whether it concerns location, 
    #   X-ray spectrum, X-ray variability, multiwavelength ratio... The weights of 
    #   each category are treated independently.
    # pba_ifnull: whether the algorithm should compute and use probabilities that 
    #   the property exist given each class: hence missing values are taken into account.
    # scale: defines the x-axis scale to use when computing the distributions of the 
    #   reference sample. For instance the histogram of a flux is better represented in logscale.
...
if ncla == 0: 
   raise IOError("There are no classes found in the input catalogue.")
...
if optimize_coeffs:
    ...
    try:
        res = differential_evolution(f1score, [(len(global_coeffs)-len(categories))*[0,1]]+(len(categories))*[[0,10]], disp=1, maxiter=100 )
    ...
    status = os.system('sed -i "s/global_coeffs:.*/global_coeffs: %s/g" %s'%(str_coeffs, configfile))
    if status > 0 : print (f"error when  sed replaces the global_coeffs {str_coeffs} in {configfile} " )
    status = os.system('sed -i "s/optimize_coeffs:.*/optimize_coeffs: 0/g" %s'%configfile)
    if status > 0 : print (f"error when sed tries changing optimize_coeffs to 0 in {configfile}")
    status = os.system('sed -i "s/save:.*/save: 1/g" %s'%configfile)
    if status > 0 : print (f"error when sed tries to change save to 1 in {configfile}")
    print('Weighting coefficients [%s] saved to %s'%(str_coeffs, configfile))
